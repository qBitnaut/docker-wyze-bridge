...
def get_ffmpeg_cmd(
    ...
    if audio and "codec" in audio:
        ...
        # Allow FFMPEG to probe AAC streams (omitting -f aac) to handle raw/ADTS differences
        fmt_flag = f"-f {audio['codec']}" if audio["codec"] != "aac" else ""
        
        audio_in = f"{thread_queue} {fmt_flag}{rate} -i /tmp/{uri}_audio.pipe"
